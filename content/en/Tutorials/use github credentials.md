...
The tutorial configures the inputs inside the **config.json** file, but run the formula without informing them as `prompt` or `stdin` \(they will be extracted automatically\). You'll find more information in the [**input configuration**](//credentials/set-credentials/) section.
...
👉 If you've completed the second tutorial, go to the third [**How to use conditional inputs**](/tutorials/conditional-inputs/)!
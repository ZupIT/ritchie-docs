...
> **Note**: The tutorial is to configure those inputs inside the **config.json** file, but run the formula without informing them as `prompt` or `stdin` \(they will be extracted automatically\). You'll find all the informations you need in the [**how to manipulate credentials**]({{< ref path="credentials" >}}) tutorial section.
...
👉 If you've completed the second tutorial, let's go to the [**intermediate task**]({{< ref path="Tutorials/Intermediate" >}})!
